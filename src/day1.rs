...
pub fn execute_day_1() -> i32 {
    let lines = get_file_contents("./input/day-1.txt");
    let mut total = 0;
    for line in lines {
...
fn get_number_2(value: &str) -> i32 {
    ...
    for number in english_numbers {
        ...
        let english_matches: Vec<_> = value
            .match_indices(english_number)
            .map(|(i, _)| i)
            .collect();
        ...
        if english_matches.len() > 0 {
            ...
            let english_match_second_index =
                english_matches.get(english_matches.len() - 1).unwrap();
